---
title: Index
updated: 2026-06-12
---

# FRM Part 2 — Study Wiki Index

Master catalog of every wiki page, grouped by topic. This file is the entry
point for answering "what do I know about X" — start here, then follow
[[wikilinks]] into the relevant pages. Do not consult `raw/` to answer
questions; everything that matters has been paraphrased into `wiki/`.

Each topic section below will fill in as lessons are ingested. Within a
lesson folder you'll typically find a lesson summary page, one page per key
concept/model, an entity/people page where relevant, and a formulas page.

## market-risk — Market Risk Measurement and Management

- Lesson 1: [[market-risk/R1 - Estimating Market Risk Measures - An Introduction and Overview/Summary|Estimating Market Risk Measures: An Introduction and Overview]]
  - [[Historical Simulation VaR]] (LO 1.a)
  - [[Parametric VaR - Normal and Lognormal]] (LO 1.b)
  - [[Expected Shortfall]] (LO 1.c)
  - [[Coherent Risk Measures and Quantiles]] (LO 1.d)
  - [[Standard Errors of Risk Measure Estimators]] (LO 1.e)
  - [[QQ Plots]] (LO 1.f)

## credit-risk — Credit Risk Measurement and Management

_No lessons ingested yet._

## operational-risk — Operational Risk and Resilience

_No lessons ingested yet._

## liquidity-treasury — Liquidity and Treasury Risk Measurement and Management

_No lessons ingested yet._

## investment-management — Risk Management and Investment Management

_No lessons ingested yet._

## current-issues — Current Issues in Financial Markets

_No lessons ingested yet._
